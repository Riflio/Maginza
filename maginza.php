...
class Maginza extends Options  {
	...
	function maginza_search_join($join) {
		...
		if( is_search() ) {
			 ...
			 $join .= 'LEFT JOIN '.$wpdb->postmeta.' ON wp_postmeta.post_id=wp_posts.ID';			
		}
		return $join;
	}
	...
	function maginza_search_where( $where )	{
		global $wpdb;
		if( is_search() ) {	
			$where = preg_replace('/post_title LIKE \'(.*?)\'\)/', 'post_title LIKE  \'$1\') OR (wp_postmeta.meta_value LIKE \'$1\') ', $where );
		}
		return $where;
	}
